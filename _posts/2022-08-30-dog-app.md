...
![config.yml](/images/dog/corgi.png)
...
The objective of this project is to build a dog image classification model using transfer learning with at least 80% accuracy. In this project we will be using InceptionV3. 
...
Please try the [Dog Breed Classification App](https://carlos-lesser-dog-app-dog-app-38wto4.streamlitapp.com/) on Streamlit, or upload a picture of your four legged pal below to see the results.
...
[Standford Dataset](http://vision.stanford.edu/aditya86/ImageNetDogs/) 
...
We can observe that my model was pretty accurate when predicting pure breeds and not that accurate on mix breeds. This is becasue the model was train with 120 pure breeds. The Brittany Spaniel, Boston Bull, and Labrador were part of does breeds. Our model would not be able to classify any new breeds outside the 120 breeds the model was trained on. This is why our model couldn't predict the mix breeds accurate.
...
## Summary 
The model performs well with 88% accuracy on the test. I was expecting to reach 70% accuracy on the model, and I was able to achieve that.  The next time I do a classification problem, I would focus more on precision, recall and F1 scores. This project was fun, and it gave me a better understanding of CNN models and transfer learning. This knowledge can be applied to different industries such as manufacturing (for defect detection), medicine (to
detect anomalies in organs or tissues), agriculture (to identify which plans need water or to spot plagues on
the crops), commerce (to identify similar products). The next step would be to make an image recognition
model to identify if the image is of a dog.
